...
export default class SearchBar extends React.Component {
  ...
  render() {
    ...
    (this.props.colors.map((color) => {
        colors.push(<div className={`${color}`.split("/")[3].split(".")[0]} onClick={(e) => this.optionClicked(e)}><img className={`${color}`.split("/")[3].split(".")[0]} src={color} alt={`${color}`.split("/")[3].split(".")[0]}></img></div>)
    }))
