...
export default class MoreInformation extends React.Component {
  render() {
    ...
    while (moreIngredients) {
      moreIngredients =
        !(this.props.drink["strIngredient" + i] === null) &&
        !(this.props.drink["strIngredient" + i] === "");
    ...
    for (let j = 1; j < i; j++) {
      if (this.props.drink["strMeasure" + j] === null)
        txt.push(
          <>
            {this.props.drink["strIngredient" + j] + " : unknown"}
            <br />
          </>
        );
      else
        txt.push(
          <>
            {this.props.drink["strIngredient" + j] +
              " : " +
              this.props.drink["strMeasure" + j]}
            <br />
          </>
        );
    }
      ...
      <div className="popup-wrapper">
        <div className="popup">
          <div className="popup-header">
            <div className="popup-title">{this.props.drink.strDrink}</div>
            <div className="popup-close">
              <i
                className="fas fa-times"
                onClick={this.props.closeMoreInformation}
              ></i>
            </div>
          </div>
